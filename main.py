...
def is_upcoming(event: dict, now: datetime.datetime) -> bool:
    """True if the public on-sale hasn't started yet (an upcoming drop).
    If there's no public on-sale time, fall back to any future presale; if
    nothing is known, keep it so a real drop is never silently hidden."""
    sales = event.get("sales", {}) or {}
    pub = parse_iso((sales.get("public") or {}).get("startDateTime"))
    if pub:
        return pub > now
    presales = sales.get("presales") or []
    starts = [parse_iso(p.get("startDateTime")) for p in presales]
    starts = [s for s in starts if s]
    if starts:
        return any(s > now for s in starts)
    return True  # unknown -> keep, let the admin decide
...
@app.get("/shows")
async def shows(q: str, country: str | None = None, upcoming: str | None = "true"):
    if not TM_KEY:
    ...
    # keep only shows whose public on-sale hasn't started yet (upcoming drops)
    total_found = len(events)
    if (upcoming or "true").lower() != "false":
        now = datetime.datetime.now(datetime.timezone.utc)
        events = [e for e in events if is_upcoming(e, now)]
    ...
    return {
        ...
        "filteredUpcoming": (upcoming or "true").lower() != "false",
        "shows": out,
    }
...
async def verify(req: VerifyReq):
    ...
    cities = ", ".join(req.cities[:40]) or "the listed markets"
    prompt = (
        "You are fact-checking a ticket-broker community post before it goes out to paying members. "
        "Accuracy is critical: members make buying decisions from this, so a wrong claim costs them money.\n\n"
        f"ARTIST / EVENT: {req.artist} {req.tour}\n"
        f"MARKETS: {cities}\n\n"
        "THE DRAFT POST TO CHECK:\n"
        f'"""\n{req.draft}\n"""\n\n'
        "Use web search to verify against current, real sources (the official artist site, Ticketmaster, "
        "and reputable news). Check especially:\n"
        "- Ticket price, and whether pricing is flat or dynamic\n"
        "- Whether there is actually a PRESALE (and the codes / dates / times) or only a public on-sale\n"
        "- On-sale and presale dates and times\n"
        "- Ticket limit per order\n"
        "- Transferability and resale rules: are tickets transferable? face-value-only? Verified Fan? "
        "any anti-scalping restrictions? (this matters most to resellers)\n"
        "- Venue / capacity if mentioned\n"
        "- Any demand or resale-price claims\n\n"
        "Then produce a clear report an admin and members can read. Format it EXACTLY like this:\n\n"
        "**VERDICT:** one line — is the draft safe to post as-is, or does it need fixes?\n\n"
        "**Checked:**\n"
        "- [OK] <claim> — <why it's confirmed> (source name)\n"
        "- [FIX] <claim> — <what's wrong and the correct info> (source name)\n"
        "- [UNVERIFIED] <claim> — couldn't confirm; verify on the official announcement\n\n"
        "**Watch-outs for members:** anything about resale limits, transfer restrictions, no-presale, "
        "Verified Fan, or price caps that changes whether this is worth buying.\n\n"
        "RULES: Never invent facts, numbers, sources, or presale codes. If you cannot verify something, "
        "mark it UNVERIFIED — do not guess. Plain language. Concise but complete."
    )
